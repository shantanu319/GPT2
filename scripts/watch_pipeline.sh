...
if [ -z "${SKIP_LAUNCH:-}" ]; then
  ...
  python3 vast/vast_train.py pipeline --dir-name "$DIR_NAME" --sft-dir-name "$SFT_DIR_NAME" \
...
SECONDS=0
while true; do
  if python3 vast/vast_train.py ssh "test -f $REMOTE/saved/$DPO_DIR_NAME/dpo_final.pt" >/dev/null 2>&1; then
  ...
  if (( SECONDS >= MAX_HOURS * 3600 )); then
    ...
    python3 vast/vast_train.py ssh "ls -t $REMOTE/saved/*/*.pt 2>/dev/null | head -5; tail -3 $REMOTE/pipeline.log 2>/dev/null" \
      2>/dev/null | tee -a "$LOG"
    python3 vast/vast_train.py pull 2>&1 | tail -3 | tee -a "$LOG"
    if [ "$DESTROY_ON_TIMEOUT" = "1" ]; then
      note "destroying instance (DESTROY_ON_TIMEOUT=1)"
      python3 vast/vast_train.py destroy 2>&1 | tail -2 | tee -a "$LOG"
    else
      note "instance still running (meter on) — stop it with: python vast/vast_train.py destroy"
    fi
    note "DONE (timeout) — latest weights in ./vast_out/saved/"
    exit 0
  fi
  tail_line=$(python3 vast/vast_train.py ssh "tail -1 $REMOTE/pipeline.log 2>/dev/null" 2>/dev/null | tail -1)
  note "poll: ${tail_line:-no pipeline.log yet}"
  if ! python3 vast/vast_train.py ssh "pgrep -f remote_pipeline.sh >/dev/null" >/dev/null 2>&1; then
