...
line. Its defaults are the dense stack (`-kda 0 -swa 0`; the KDA hybrid measured
about half of dense throughput on an A100 and has never been trained at scale),
`-muon_per_head 1`, a 20% WSD decay, and a checkpoint every 2000 steps; `-attn_res`
and `-loops` stay off. A restart passes the newest `ckpt_step*.pt` as `-resume`, so
a preempted box picks the schedule up where it stopped rather than at step 0.
`--anneal-prefix` pulls a second prepared corpus and serves it through the decay
phase (`-anneal_dir`); it is only ever fetched, never tokenized on the GPU box.

    python3 vultr/vultr_train.py prep --max-train-docs 40000000 --prefix corpus-40b
    python3 vultr/vultr_train.py prep --mix anneal --max-train-docs 5000000 --prefix anneal-6b
    python3 vultr/vultr_train.py pipeline --corpus-prefix corpus-40b --anneal-prefix anneal-6b
